...
export default function Footer() {
  const { schoolIdentity } = useApp();
  const mapEmbedUrl = getGoogleMapsEmbedUrl(schoolIdentity);
  const headerName = schoolIdentity.schoolShortName || schoolIdentity.schoolName || 'SMPN 1 Genteng';
  const legalName = schoolIdentity.legalName || schoolIdentity.schoolName || 'SMP Negeri 1 Genteng';

  const copyrightLine = (() => {
    const year = schoolIdentity.showCurrentYear ? `© ${new Date().getFullYear()} ` : '';
    const school = legalName;
    const custom = schoolIdentity.copyrightText ? ` — ${schoolIdentity.copyrightText}` : '';
    return `${year}${school}${custom}`;
  })();

  const rightLine = (() => {
    const base = schoolIdentity.legalNotice || '';
    if (schoolIdentity.developerName) {
      const devLink = schoolIdentity.developerUrl
        ? `<a href="${schoolIdentity.developerUrl}" target="_blank" rel="noopener noreferrer" class="text-primary-300 hover:text-white underline underline-offset-2 transition-colors">${schoolIdentity.developerName}</a>`
        : schoolIdentity.developerName;
  ...
  return (
    <footer className="text-white" style={{ backgroundColor: schoolIdentity.footerBackgroundColor }}>
            ...
            <div className="flex items-center space-x-3 mb-4">
              <div className="w-9 h-9 sm:w-10 sm:h-10 bg-gradient-to-br from-primary-400 to-primary-500 rounded-xl flex items-center justify-center">
                {schoolIdentity.showLogo && schoolIdentity.schoolLogo ? (
                  <img src={schoolIdentity.schoolLogo} alt={`Logo ${headerName}`} className="h-full w-full object-contain rounded-xl bg-white p-1.5" />
                ) : (
                  <GraduationCap className="h-5 w-5 sm:h-6 sm:w-6 text-white" />
                )}
              </div>
              <div>
                <h3 className="text-base sm:text-lg font-bold">{headerName}</h3>
                <p className="text-primary-300 text-[10px] sm:text-xs">{schoolIdentity.schoolTagline || 'Kabupaten Banyuwangi'}</p>
              </div>
            </div>
            <p className="text-primary-200 text-xs sm:text-sm leading-relaxed mb-4">
              {schoolIdentity.footerDescription}
            </p>
            <div className="flex space-x-3">
              {schoolIdentity.facebook && (
                <a href={schoolIdentity.facebook} target="_blank" rel="noopener noreferrer" className="w-8 h-8 sm:w-9 sm:h-9 bg-primary-800 hover:bg-primary-700 rounded-lg flex items-center justify-center transition-colors" aria-label="Facebook">
              ...
              {schoolIdentity.instagram && (
                <a href={schoolIdentity.instagram} target="_blank" rel="noopener noreferrer" className="w-8 h-8 sm:w-9 sm:h-9 bg-primary-800 hover:bg-primary-700 rounded-lg flex items-center justify-center transition-colors" aria-label="Instagram">
              ...
              {schoolIdentity.youtube && (
                <a href={schoolIdentity.youtube} target="_blank" rel="noopener noreferrer" className="w-8 h-8 sm:w-9 sm:h-9 bg-primary-800 hover:bg-primary-700 rounded-lg flex items-center justify-center transition-colors" aria-label="YouTube">
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              ...
              <li className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm text-primary-200">
                ...
                <span>{schoolIdentity.hours}</span>
              </li>
            </ul>
          </div>
